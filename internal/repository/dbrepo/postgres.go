...
//InsertRoomRestriction inserts a room restriction into the database, usually because of reservation, maintenace, etc
...
// in other words, if there are restrictions found, return false, otherwise if none are found return true
// the proposed start date being earlier than end_date of currently checked room and proposed end date being
// later than currently checked start date is very efficient and clean way of making sure that you find any possible
// restrictions.  
// room_id check is to make sure you're actually checking the correct room
...
//GetRoomByID gets a room by inserting a room id
...
//GetUserByID gets a users login info by id from data base
...
//UpdateUser takes basic user info and pastes it into an existing user 
...
func (m *postgresDBRepo) Authenticate(email, testPassword string) (int, string, error) {
	...
	log.Println("id, password: ", id, hashedPassword)
	if err != nil {
		log.Println("err: ", err)
		return id, "", err
	}
...
//All new reservations returns a slice of all new reservations
